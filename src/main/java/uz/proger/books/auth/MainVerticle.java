...
public class MainVerticle extends AbstractVerticle {
  ...
  private void configureRoutes(Router router, WebClient webClient, OAuth2Auth oauth2Auth) {
    ...
    router.route("/logout")
      .handler(ctx -> {
        ...
        ctx.redirect(oauth2Auth.endSessionURL(ctx.user()));
      });
  }
  ...
  private void handleUserPage(RoutingContext ctx) {
    ...
    String username = user.attributes().getJsonObject("accessToken").getString("preferred_username");
  ...
  private void handleAdminPage(RoutingContext ctx) {
    ...
    AuthorizationProvider auth = KeycloakAuthorization.create();

    auth.getAuthorizations(user)
      .onSuccess(v -> {
        if (
...
//      }
//
//      String username = user.principal().getJsonObject("idToken").getString("preferred_username");
//
//      String content = String.format("<h1>Admin Page: %s @%s</h1><a href=\"/protected/\">Protected Area</a>", username, Instant.now());
//      respondWithOk(ctx, "text/html", content);
//    });
  ...
  private void handleGreet(RoutingContext ctx) {
    ...
    String username = user.attributes().getJsonObject("accessToken").getString("preferred_username");
    String displayName = user.attributes().getJsonObject("accessToken").getString("name");
